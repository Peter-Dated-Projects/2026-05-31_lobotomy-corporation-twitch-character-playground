...
| [facing-tracked-but-sprite-never-flipped](facing-tracked-but-sprite-never-flipped.md) | Character tracks _facing and flips it at edges/pauses, but the renderer never flips the sprite, so characters always face the sheet default direction; fix is a pygame.transform.flip in scene.py keyed off exposed facing with a WALK_THRESHOLD deadzone. | current |
